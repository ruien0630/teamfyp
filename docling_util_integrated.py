...
from langchain_chroma import Chroma # baseline version changed (from langchain.vectorstores import Chroma)
...
# Function to convert PDF with image annotation
def convert_pdf_with_image_annotation(input_doc_path: str, ai_service_provider: str = "google") -> DocumentConverter:
    ...
    # Configure picture description options based on AI service provider
    if ai_service_provider.lower() in ["google", "openai"]:
        ...
        # Set up picture description options
...
def create_chroma_vectordb(
    ...
    Args:
        ...
        text_splitter_choice (str): The type of text splitter to use. Supported choices are
                                    "CharacterTextSplitter", "TokenTextSplitter", and "ContentAwareSplitting".
    ...
    try:
        ...
        # Step 2: Initialize and configure the chunking method
        if text_splitter_choice == "ContentAwareSplitting":
            # Use Docling's HierarchicalChunker for content-aware splitting
            max_tokens = splitter_para.get('max_tokens', 800)
            chunker = HierarchicalChunker(max_tokens=max_tokens)
            
            # Convert and chunk documents directly
            chunks = []
            for file_path in file_paths:
                doc_result = converter.convert(source=file_path)
                chunk_iter = chunker.chunk(doc_result.document)
                for chunk in chunk_iter:
                    # Convert DocChunk to LangChain Document with simplified metadata
                    doc = Document(
            ...
            # For other splitters, use standard loader approach
            loader = DoclingLoader(file_paths, converter=converter)
            documents = loader.load()

            # Ensure source filename metadata exists for filtering
            for doc in documents:
                source_path = doc.metadata.get("source", "")
                doc.metadata["source_filename"] = os.path.basename(source_path)

            # Initialize and configure the text splitter
            ...
            if text_splitter_choice == "CharacterTextSplitter":
                ...
                raise ValueError(
                    ...
                    "Supported splitters are 'CharacterTextSplitter', 'TokenTextSplitter', and 'ContentAwareSplitting'."
                )        
