...
Answer: """
    ...
    try:
        # Best retrieval method recommended by team
        search_kwargs = {
            "k": k,
            "fetch_k": fetch_k,
...
def ask_question(qa_chain, question):
    ...
    # DEBUG: Print number of sources retrieved
    print(f"DEBUG: Retrieved {len(sources)} source documents for query: '{question}'")
    ...
    #return response
    # NEW: Extract both the source filename AND the specific text chunk
    detailed_sources = []
    for doc in sources:
        detailed_sources.append({
            "source": doc.metadata.get("source", "Unknown"),
            "content": doc.page_content  # 👈 This pulls the specific text chunk
        })
