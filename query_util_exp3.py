...
def setup_qa_chain(
    ...
    use_ollama=True,  
    ollama_model="gemma3:4b",  # Updated for gemma3 4B model
    temperature=0.1 #baseline 0.1
):
    """
    ...
    Args:
        ...
        use_local_path (bool): If True, the function will load the vector store from
                               ...
                               vector_object.
        model_id (str): The model to use. Options: 'microsoft/phi-3-medium-128k-instruct', 'ibm/granite-3-8b-instruct'.
        use_ollama (bool): If True, use local Ollama instead of IBM Watsonx.
        ollama_model (str): The name of the Ollama model (default: "phi3:medium").
    ...
    if use_ollama:
        try:
            ...
            raise Exception(f"Error: Run 'ollama pull {ollama_model}' first. Details: {e}")
    else:
        # Fallback to Watsonx if needed
        llm = WatsonxLLM(
